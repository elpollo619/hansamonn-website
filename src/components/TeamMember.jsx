    ...
    >
      ...
      <div className="relative overflow-hidden h-64">
        ...
        <div className="absolute bottom-4 left-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center gap-3">
          ...
          <Link
            to={`/team/${member.slug}`}
            className="flex-1 bg-white/90 rounded-full py-1.5 px-3 flex items-center justify-center gap-1 hover:bg-white transition-colors text-xs font-medium text-blue-600"
          >
      ...
      <div className="p-5">
        ...
        <Link
          to={`/team/${member.slug}`}
          className="inline-flex items-center gap-1 text-blue-600 text-xs font-medium hover:gap-2 transition-all"
        >
          Profil ansehen <ArrowRight size={12} />
        </Link>
      </div>
